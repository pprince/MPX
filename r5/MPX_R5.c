...
# include <dos.h> 
	...
	int com_open (int *eflag_p, int baud_rate){
		...
		mask = mask & 0xEF; //0010 0001 and not(0001 0000) (which is 1110 1111) = 0010 0001
	...
	void level2Write(){
		...
		if((dcb.trans)<*(dcb.Ocount)){
			...
			mask = inportb(INTEN);
			mask = mask & 0xFD;  //0000 1101 and  not 0000 0010 (1111 1101) = 0000 1101 = 0x0D
			outportb(INTEN, mask);
		}		
	}
	...
	int com_read (char *buf_p,int *count_p){
		...
		*dcb.ocFlagPtr=OPEN;
		...
		while(((dcb.Ibuff-1)!='\r')&&(dcb.count!=0)&&((*dcb.Icount)<=(dcb.request))){ // check for return key, full buffer, not overwrite unread chars
			...
			dcb.Ibuff++;
			dcb.put++;
			dcb.put==(dcb.put)%512; // check this
			dcb.count--;
		}
		enable();

		if(!((dcb.request)< *(dcb.Icount))){ // check to see if full
